$(document).ready(function(){var lastId,topMenu=$("#top-menu"),topMenuHeight=topMenu.outerHeight()+15,menuItems=topMenu.find("a"),scrollItems=menuItems.map(function(){var item=$($(this).attr("href"));if(item.length){return item;}});$(window).scroll(function(){var fromTop=$(this).scrollTop()+topMenuHeight;var cur=scrollItems.map(function(){if($(this).offset().top<fromTop)
return this;});cur=cur[cur.length-1];var id=cur&&cur.length?cur[0].id:"";if(lastId!==id){lastId=id;menuItems.parent().removeClass("menu-active").end().filter("[href='#"+id+"']").parent().addClass("menu-active");}});$(function stickyNav(){var nav=$('#top-menu');if(nav.length){$(window).scroll(function(){var threshold=300;if($(window).scrollTop()>=threshold)
$('#top-menu').addClass('moveMenuDown');else
$('#top-menu').removeClass('moveMenuDown');var check=$(".content").height()- $("#top-menu").height();if($(window).scrollTop()>=check)
$('#top-menu').addClass('moveMenuUp');else
$('#top-menu').removeClass('moveMenuUp');});};});});$(document).ready(function(){$('a[href*=#]').bind("click",function(e){var anchor=$(this);var locate=$.attr(this,'href').substr($.attr(this,'href').indexOf('#')+1);$('html, body').stop().animate({scrollTop:$('[id="'+ locate+'"]').offset().top- 50},800,"swing");$(this).delay(800).queue(function(){window.location.hash=locate;});e.preventDefault();});});$(document).ready(function(){$('.mobile-menu-btn').click(function(){$('.mobile-menu-dropdown').toggleClass('showMobileMenu');});$('.post-content img.img-responsive').attr('data-action','zoom');});+function($){'use strict';function transitionEnd(){var el=document.createElement('bootstrap')
